...
export const makeRemito = (req,res) => {
    try {
        const data = req.body.dataRemito.Payload.properties;
        ...
        res.setHeader("Content-Disposition", `attachement; filename=remito-${numero}.pdf`);
        ...
        remito.font('Courier-Bold').fontSize(10).text (`0003 - ${numero}`,50,50,{align:"right"});
        remito.moveDown(2);
        remito.fontSize(9)
        ...
        remito.font('Courier').fontSize(8).text(`${data.razon_social}`,50,130);
        remito.text(`${data.address}`,50,138);
        remito.text(`${data.zip} - ${data.city}`,50,146);
        remito.text("Provincia",50,154);
        remito.text(`IVA: ${data.condicion_frente_al_iva}`,50,162);
        remito.text("Cond. de Venta: cta.cte 30 días",50,170);

        remito.text(`Cliente: ${data.numero_cliente_infouno}`,350,130,{align:"left"});
        remito.text(`Kilos:`,350,138,{align:"left"});
        remito.text(`Bultos:`,350,146,{align:"left"});
        remito.text(`CUIT: ${data.cuit___tax_id}`,350,154,{align:"left"});
        ...
        for (let index = 0; index < items.length; index++) {
            ...
            if(req.body.lineItems[index].properties.hs_product_id !== '2050255227'/**Costo de gestion**/ && req.body.lineItems[index].properties.hs_product_id !=='2452159355'/**seguro FS */){
